...
def getDetectedFrame(img):
    global info_det, giveway_det, mandatory_det, priority_det, prohibitory_det, stop_det, warning_det
    ...
    if len(dets_info) > 0:
        for k,d in enumerate(dets_info):
            cv2.rectangle(img, (d.left(), d.top()), (d.right(), d.bottom()), (255,191,0), 2)
    if len(dets_giveway) > 0:
        for k,d in enumerate(dets_giveway):
            cv2.rectangle(img, (d.left(), d.top()), (d.right(), d.bottom()), (0,191,255), 2)
    if len(dets_manda) > 0:
    ...
    if len(dets_priority) > 0:
        for k,d in enumerate(dets_priority):
            cv2.rectangle(img, (d.left(), d.top()), (d.right(), d.bottom()), (0,255,255), 2)
    if len(dets_proh) > 0:
